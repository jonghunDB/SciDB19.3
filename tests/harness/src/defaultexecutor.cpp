...
int DefaultExecutor :: runSciDBquery (const string &queryString, const ErrorCommandOptions *errorInfo)
{
	...
	try
	{
		...
		else if (queryResult.selective && _ignoredata_flag == true)
		{
			...
			while (!iterators[0]->end())  // DJG should be okay, even if iterators[0] is EBM, right?
			{
                            for (const auto& attr : attrs)
                            {
                                if (iterators[attr.getId()]) {
                                    totalSize += iterators[attr.getId()]->getChunk().getSize();
...
{
        LOG4CXX_INFO (_logger, "Echo: " << args);
...
{
        LOG4CXX_INFO (_logger, "Executing Command " << section.cmd);
...
int DefaultExecutor :: execute (InfoForExecutor &ie)
{
	time_t rawtime;
	time ( &rawtime );
	std::string nowStr(iso8601_localtime (&rawtime));
