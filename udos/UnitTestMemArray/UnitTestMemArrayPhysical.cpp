     ...
     */
        ...
        )
        {
            ...
            try
            {
                ...
                stringstream ss;
                ss << "Map start coordinate : "<< start << " , " << "Map end coordinate : " << end <<" , "
                << "Map chunk interval : "<< chunkInterval <<" , " <<" , "   << "Map density : "<< percentNullValue <<" , "
                << "time : "<< checktime  <<" , "<<"time(s) : " << checktime/ CLOCKS_PER_SEC << endl;



                ofstream txtout("/home/jh/scidbsource19.3/udos/UnitTestMemArray/outfileMap.txt",ios::app);
...
/*        void makeNDArrayIter(std::shared_ptr<Query>& query,
                TypeId const& type,
                Coordinates start,
                Coordinates end,
                uint32_t chunkInterval,
...
\
                stringstream ss;
                ss << "Iterator start coordinate : "<< start << " , " << "Iterator end coordinate : " << end <<" , "
                   << "Iterator chunk interval : "<< chunkInterval <<" , "   << "Iterator density : "<< percentNullValue <<" , "
                   << "time : "<< checktime << " , " <<"time(s) : " << checktime/ CLOCKS_PER_SEC << endl;

                ofstream txtout("/home/jh/scidbsource19.3/udos/UnitTestMemArray/outfileIter.txt", ios::app);
        ...
        std::shared_ptr<Array> execute(vector< std::shared_ptr<Array> >& inputArrays, std::shared_ptr<Query> query)
        {
            ...
            {
                Value const& start = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[i])->getExpression()->evaluate();
                if(start.isNull()||start.getInt64() > dims[i].getEndMax())
                {
                    startC[i] = dims[i].getEndMax();
                }
                else
                {
            ...
            for(size_t i=0 ; i <nDims ;i++)
            {
                Value const& end = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[i + nDims])->getExpression()->evaluate();
                if(end.isNull()||end.getInt64() > dims[i].getEndMax())
                {
                    endC[i] = dims[i].getEndMax();
                }
                else
                {
                    endC[i] = end.getInt64();
            ...
            Value const&  ndimms = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[2*nDims])->getExpression()->evaluate();


            Value const&  chunkInterval = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[1+ 2*nDims])->getExpression()->evaluate();
            Value const&  density = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[2+ 2*nDims])->getExpression()->evaluate();
            Value const&  threshold  = ((std::shared_ptr<OperatorParamPhysicalExpression>&)_parameters[3+ 2*nDims])->getExpression()->evaluate();
            ...
            //make1DArrayIter(query, attribute type, start coordinate, end coordinate, threshold);
            if (query->isCoordinator())
            {
                srand(static_cast<unsigned int>(time(NULL)));

                //threshold =0 iterator
                if(threshold.getUint32() == 0)
                {
                    make1DArrayIter(query, TID_INT64, startC[0], endC[0],chunkInterval.getUint32(), 8,density.getInt32());
                }

                //threshold =1 map
                else if(threshold.getUint32() == 1 )
                {
                    make1DArrayMap(query, TID_INT64, startC[0], endC[0],chunkInterval.getUint32(), 8 , density.getInt32());
                }

                //make1DArrayIter(query, TID_INT64, startC[0], endC[0],chunkInterval.getUint32(), 8);
