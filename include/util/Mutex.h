...
private:
 ...
 private:
  ...
  public:
    ...
    void lock(const WaitTimerParams& wtp, bool logOnCompletion = false)
    {
        {
            ...
            ScopedWaitTimer timer(wtp, logOnCompletion);
    ...
    void lock(perfTimeWait_t tw, bool logOnCompletion = false)
    {
        {
            // destruction updates the timing of tw
            // note: we are purposely timing only the time until lock
...
public:
     ...
     */
        ScopedMutexLock(Mutex& mutex,
                        perfTimeWait_t tw = PTW_SML_DFLT,
                        bool logOnCompletion = false)
            : _mutex(mutex)
	{
        _mutex.lock(WaitTimerParams(tw,1,NULL), logOnCompletion);
	}
     ...
     */
	ScopedMutexLock(Mutex& mutex,
                        const WaitTimerParams& wtp,
                        bool logOnCompletion = false)
            : _mutex(mutex)
	{
        _mutex.lock(wtp, logOnCompletion);
	}
