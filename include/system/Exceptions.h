    ...
    do {                                                                \
        ...
        if (!cond) {                                                    \
            ...
            throw SYSTEM_EXCEPTION(scidb::SCIDB_SE_INTERNAL,            \
                ...
                << #_cond_ << __FILE__ << __LINE__ << ss.str();         \
        }                                                               \
    } while (false)

/**
 * Expect that _stmt_ will not throw.  In the event that it throws
 * and the product is built as DEBUG, trigger an assertion.  In the event
 * that it throws and the product is built as RELEASE, invoke the
 * callback provided via _onExceptionCallback_ to provide sane handling
 * in release build environments.
 */
#   define SHOULD_NOT_THROW(_stmt_, _onExceptionCallback_)       \
    do {                                                         \
        try {                                                    \
            _stmt_;                                              \
        }                                                        \
            ...
            assert(false);                                       \
            _onExceptionCallback_(exceptionText.c_str());        \
        }                                                        \
        catch (...) {                                            \
            assert(false);                                       \
            _onExceptionCallback_("unknown exception");          \
        }                                                        \
    } while (false)
