...
public:
    virtual DelegateArrayIterator* createArrayIterator(const AttributeDesc& id) const
    {
        ...
        SCIDB_ASSERT(_projection[id.getId()] < inputArray->getArrayDesc().getAttributes().size());
        const auto& projectedAttr = inputArray->getArrayDesc().getAttributes().findattr((size_t)_projection[id.getId()]);
        return new DelegateArrayIterator(*this, id, inputArray->getConstIterator(projectedAttr));
    }
