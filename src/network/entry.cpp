...
bool startInstance(const std::string& address, uint16_t port)
{
    ...
    if (registerInstance) {
        ...
        LOG4CXX_DEBUG(logger, "Registered instance # " << Iid(newInstanceID));
    }
    else {
        if (selfInstanceID == INVALID_INSTANCE) {
...
void runSciDB()
{
   ...
   {
      LOG4CXX_ERROR(logger, "runSciDB: failed to initialize server configuration");
      printPrefix();
   ...
   LOG4CXX_TRACE(logger, "runSciDB: making job thread pool");
...
//
   ...
   {
       LOG4CXX_ERROR(logger, "runSciDB: Error during SciDB initialization: " << e.what());
       errorCode = 1;
   }
   ...
   {
       LOG4CXX_ERROR(logger, "runSciDB: Error during SciDB execution: " << e.what());
       errorCode = 1;
   }
