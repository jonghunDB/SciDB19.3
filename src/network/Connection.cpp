...
void Connection::start()
{
    ...
    _networkManager.getIOService().post(boost::bind(&Connection::readMessage,
                                             shared_from_this()));
}
...
void Connection::readMessage()
{
   ...
   boost::asio::async_read(_socket,
                           ...
                           boost::bind(&Connection::handleReadMessage, shared_from_this(),
    ...
    size_t bytes_transferr)
{
   ...
   async_read(_socket,
              ...
              boost::bind(&Connection::handleReadRecordPart,
                             ...
                             NetworkManager::MessageQueueType mqt)
{
   ...
   _networkManager.getIOService().post(boost::bind(&Connection::pushNextMessage,
                                            shared_from_this()));
}
                             ...
                             NetworkManager::MessageQueueType mqt)
{
    ...
    if (connStatus) {
        ...
        _networkManager.getIOService().post(boost::bind(&Connection::publishQueueSize,
                                                 shared_from_this()));
    }
}
...
void Connection::pushNextMessage()
{
   ...
   boost::asio::async_write(_socket, constBuffers,
                     boost::bind(&Connection::handleSendMessage,
                          shared_from_this(),
                          boost::asio::placeholders::error,
                          boost::asio::placeholders::bytes_transferred,
                          msgs, size));
...
void Connection::authDone()
{
    if (::pthread_equal(::pthread_self(), _networkManager.getThreadId())) {
        ...
        _networkManager.getIOService().post(
            boost::bind(&Connection::authDoneInternal, shared_from_this()));
    }
}
...
{
   _networkManager.getIOService().post(boost::bind(&Connection::connectAsyncInternal,
...
{
   _networkManager.getIOService().post(boost::bind(&Connection::disconnectInternal,
                                            shared_from_this()));
}
