...
namespace scidb
{
    ...
    std::shared_ptr<ConstArrayIterator> MemArray::getConstIteratorImpl(const AttributeDesc& attr) const
    {
         ...
         // at this point.
         // This codepath only operates on transient arrays as other array types start
         // their lives eventually as a DBArray.  DBArray has code already to handle
         // this same case, but for persistent arrays.
         if (!attr.isEmptyIndicator() && desc.isTransient() && iter->end()) {
             std::shared_ptr<MemArray> owner;
             owner = (const_cast<MemArray*>(this))->shared_from_this();
             std::shared_ptr<Query> query = Query::getValidQueryPtr(_query);
             iter = std::make_shared<SyntheticIterator>(owner, attr, query);
         }
