...
namespace scidb {
        ...
        try {
            ...
            while (!_sortIters.end() && !limitReached)
            {
                ...
                if (currentSize > _sorter._memLimit)
                {
                    ...
                    buffer->truncate();
                    {
                        std::shared_ptr<Array> ma(std::make_shared<MemArray>(baseBuffer->getArrayDesc(),
                                                                        getQuery()));
                                                   ...
                                                   )
    {
        ...
        // Timing for Sort
        LOG4CXX_DEBUG(logger, "[SortArray] Sort for array " << getExpandedOutputSchema().getName() << " begins");
        ...
        {
            _sortIterators[i] =
                std::shared_ptr<SortIterators>(std::make_shared<SortIterators>(_input, i, numJobs));
            _waitingJobs[i] =
                std::shared_ptr<Job>(std::make_shared<SortJob>(this,
                                            query, phyOp,
                                            i,
        ...
        if (_results.size() > 1)
        {
            ...
            lastJob->wait(Job::CAN_RAISE | Job::CAN_MULTIWAIT);
        }
