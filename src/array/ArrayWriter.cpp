...
#   define Fprintf(_fp,  _vargs...)             \
    ...
    static struct FormatInfo {
        ...
        bool coords;            // Has embedded coordinates?
    } supportedFormats[] = {
        {"(binary)", false},    // Dummy entry for '(...)' binary template
        {"csv", false},
        {"csv+", false},
        {"dcsv", false},
        {"dense", true},
    ...
    FormatInfo const* findFormatInfo(std::string const& format)
    {
        if (format.empty()) {
            return nullptr;
        }
        ...
        // them.  We only want to search for the base format.
        string::size_type colon = baseFormat.find(':');
        if (colon != string::npos) {
            baseFormat = baseFormat.substr(0, colon);
        }
        const char* fmt = baseFormat.c_str();

        // Scan format info table for a match.
        for (unsigned i = 0; i < NUM_FORMATS; ++i) {
            if (0 == ::strcasecmp(fmt, supportedFormats[i].cname)) {
    ...
    bool ArrayWriter::hasCoordinates(const string& format)
    {
        FormatInfo const* fi = findFormatInfo(format);
        ASSERT_EXCEPTION(fi, "Should have validated the format by now!");
        return fi->coords;
    }
                                   ...
                                   std::string const& format)
    {
        ...
        vector< std::shared_ptr<ConstArrayIterator> > arrayIterators(attrsCount);
        vector< std::shared_ptr<ConstChunkIterator> > chunkIterators(attrsCount);
        ...
        chunkPos[nDimensions-1] -= dims[nDimensions-1].getChunkInterval();
        {
            ...
            while (!arrayIterators[attrs.firstDataAttribute().getId()]->end()) {
                ...
                for (const auto& attr : attrs) {
                    ...
                    chunkIterators[attr.getId()] = std::shared_ptr<ConstChunkIterator>(
                         new CompatibilityIterator(chunkIterators[attr.getId()], isSparse));
                }
...
#       undef PAD
                               ...
                               FILE* fp)
    {
        ...
        if (fp) {
            f = fp;
        } else if (file == "console" || file == "stdout") {
            f = stdout;
        } else if (file == "stderr") {
            f = stderr;
        } else {
