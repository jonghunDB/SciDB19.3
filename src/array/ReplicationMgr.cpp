...
void ReplicationManager::wait(const std::shared_ptr<Item>& item)
{
    ...
    Event::ErrorChecker ec = bind(&ReplicationManager::keepWaiting, item);
...
{
    if (desc.getDistribution()->getDistType() == dtReplication) {
                                     ...
                                     std::shared_ptr<Query> const& query)
{
    ...
    if (desc.getDistribution()->getDistType() == dtReplication || // primary on all instances
