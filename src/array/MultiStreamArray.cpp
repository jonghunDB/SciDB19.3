                                      ...
                                      const AttributeID attId)
{
    ...
    for (list<size_t>::iterator it=currPartialStreams.begin();
        ...
        if (_isDataframe) {
            LOG4CXX_INFO(logger, funcName << "index=" << index << " attId=" << attId << ", stream=" << stream
                          << ", next chunk at " << CoordsToStr(_currMinPos[attId])
                          <<", next->getSize()=" << next->getSize() << ", err=" << static_cast<bool>(err));
        } else {
            LOG4CXX_DEBUG(logger, funcName << "index " << index << " attId=" << attId << ", stream=" << stream
                          << ", next (chunkBody)=" << next << " at " << CoordsToStr(_currMinPos[attId])
                          <<", next->getSize(): " << next->getSize() << ", err=" << static_cast<bool>(err));
        }
