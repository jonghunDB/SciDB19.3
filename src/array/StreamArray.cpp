...
namespace scidb
{
    ...
    : _array(arr), _attId(arr.getArrayDesc().getAttributes().findattr(attr)), _currentChunk(NULL)
    {
...
#define OUTPUT(VAR) " " << STRINGIFY(VAR) << ": " << (VAR)
    ...
    void StreamArrayIterator::moveNext()
    {
        ...
        if (_array.currentBitmapChunk == NULL ||
            ...
            if(_array.currentBitmapChunk && _array.currentBitmapChunk->getAttributeDesc().getId() != _attId.getId()) {
                LOG4CXX_TRACE(logger, "StreamArrayIterator::" << __func__
