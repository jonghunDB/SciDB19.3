    ...
    public:
        ...
        DelegateArrayIterator* createArrayIterator(const AttributeDesc& id) const override
        {
            if (id.getId() == _dstAttributeId)
            {
                ...
                const auto& attr = inputArray->getArrayDesc().getAttributes().findattr(_sourceAttributeId);
                return new DelegateArrayIterator(*this, id, inputArray->getConstIterator(attr));
            }
            else if (id.getId() == _dstAttributeId+1)
            {
                //client must be asking for the empty tag, whose id is now shifted up by one
                auto attrToTheLeft = inputArray->getArrayDesc().getAttributes().find(id);
                --attrToTheLeft;
                return new DelegateArrayIterator(*this, id, inputArray->getConstIterator(*attrToTheLeft));
            }
