...
namespace scidb
{
   ...
   : _array(arrayIterator.array),
      ...
      _nextValue(TypeLibrary::getType(_chunk.getAttributeDesc().getType()))
    {
        ...
        {
            const auto* eAttrId = _array._inputArray->getArrayDesc().getEmptyBitmapAttribute();
            assert(eAttrId);
    ...
    {
        const AttributeDesc& a = _array._inputArray->getArrayDesc().getAttributes().findattr(_array._inputAttrIDs[_attrID]);
    ...
    std::shared_ptr<ConstArrayIterator> WindowArray::getConstIteratorImpl(const AttributeDesc& attr) const
    {
        ...
        {
            return std::shared_ptr<ConstArrayIterator>(new WindowArrayIterator(*this,
                                                                               attr,
                                                                               *_inputArray->getArrayDesc().getEmptyBitmapAttribute(), _method));
        }

        const auto& outAttrDesc = _inputArray->getArrayDesc().getAttributes().findattr(_inputAttrIDs[attr.getId()]);
