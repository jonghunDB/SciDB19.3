...
public:
    ...
    {
        return std::shared_ptr<ConstArrayIterator>(new QuantileArrayIterator(*this, _desc, _buckets, attr.getId(), _grouping, _liveChunks));
    }
  ...
  public:
    ...
    std::shared_ptr<Array> execute(std::vector< std::shared_ptr<Array> >& inputArrays, std::shared_ptr<Query> query)
    {
        ...
        if (groupBy.size()==0) {
            ...
            {
                ArrayDistPtr psHashShiftedDist = ArrayDistributionFactory::getInstance()->construct(dtHashPartitioned,
                                                                                                    DEFAULT_REDUNDANCY,
                                                                                                    std::string(),
                                                                                                    i);
                rankArray = redistributeToRandomAccess(rankArray,
