...
private:
    ...
    std::shared_ptr<Array> execute(const std::shared_ptr<PhysicalQueryPlanNode>& node, const std::shared_ptr<Query>& query, int depth);
    void preSingleExecute(const std::shared_ptr<PhysicalQueryPlanNode>& node, const std::shared_ptr<Query>& query);
    void postSingleExecute(const std::shared_ptr<PhysicalQueryPlanNode>& node, const std::shared_ptr<Query>& query);
    // Synchronization methods
    /**
     * Worker notifies coordinator about its state.
     * Coordinator waits for worker notifications.
     */
    void notify(const std::shared_ptr<Query>& query, uint64_t timeoutNanoSec=0);
...
void QueryProcessorImpl::optimize(const std::shared_ptr<Optimizer>& optimizer, const std::shared_ptr<Query>& query, bool isDdl)
{
    ...
    query->getCurrentPhysicalPlan()->inferDistType(query);
...
// Recursive method for executing physical plan
std::shared_ptr<Array> QueryProcessorImpl::execute(const std::shared_ptr<PhysicalQueryPlanNode>& node, const std::shared_ptr<Query>& query, int depth)
{
    Query::validateQueryPtr(query);
