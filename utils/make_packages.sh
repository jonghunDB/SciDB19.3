...
if [ "$type" == "deb" ]; then
    ...
    if [ $target != "insource" ]; then
        ...
        if [ "$target" == "local" ]; then
            ...
            python ${scidb_src_dir}/utils/chroot_build.py -u -d "${distro}" -r "${result_dir}" -t "${build_dir}" -s "${build_dir}"/${DSC_FILE_NAME} -j${jobs} || die chroot_build.py failed
    ...
    if [ $target != "insource" ]; then
        ...
        if [ "$target" == "local" ]; then
            ...
            python ${scidb_src_dir}/utils/chroot_build.py -u -d "${distro}" -r "${result_dir}" -t "${build_dir}" -s "${build_dir}"/SRPMS/${SCIDB_SRC_RPM} || die chroot_build.py failed
