...
class SshRunner(object):
    ...
    def __init__(self, user=None, host=None, wdir=None,
                 options=None, port=None, ssh_always=None):
        ...
        self.is_local = not ssh_always and is_local_host(self.host)
        self.prefix = ['ssh']
        if port is None:
            self.port = None
        else:
        ...
        if options:
            for k, v in options.items():
                self.prefix.extend(['-o', '%s=%s' % (k, v)])
        self.prefix.append('@'.join((self.user, self.host)))
        self.path = "PATH=/bin:/usr/bin:/sbin:/usr/sbin"
        self.returncode = 0

    def __repr__(self):
        return "{0}(user='{1}', host='{2}', port={3}, wdir='{4}')".format(
            type(self).__name__, self.user, self.host, self.port, self.wdir)
