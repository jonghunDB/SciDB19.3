...
from scidblib.util import py_cast
...
def get_process_info(servers=None, server_id=None, sorted=True):
    ...
    Note: We use server_id instead of _ctx.server_id to avoid
    conflating a --server-id=X option with whether or not the
    subcommand wants to limit the returned process info.  It's quite
    possible to supply --server-id=X but still want this function to
    return all cluster process info.
    """
    if servers is None:
        servers = _ctx.scidb_servers
    assert servers, "Server list is empty?!"
    if server_id is not None:
        servers = [x for x in servers if x.sid == server_id]
    if not servers:
        raise AppError("Invalid server id %s" % _ctx.server_id)
