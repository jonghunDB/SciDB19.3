...
class UbuntuChroot():
    ...
    def __init__(self, release, arch, temp_dir):
        ...
        if self.release == "trusty":
            self.scidb_3rdparty_mirror='deb https://downloads.paradigm4.com/ 3rdparty/19.3/ubuntu14.04/'
            ...
            self.mkl='deb https://apt.repos.intel.com/mkl all main'
        else:
            err("Release %s not recognized." % self.release)
        self.mirror = '|'.join([(self.ubuntu_mirror % release), self.scidb_3rdparty_mirror, self.compiler_mirror, self.java_mirror, self.mkl])
    ...
    def exe(self, command):
        pbargs = ['pbuilder', '--execute', '--', command,
            '--basetgz', self.tgz,
            '--distribution', self.release,
            '--othermirror', self.mirror,
            '--buildplace', self.buildplace,
            ...
            '--aptcache', '',
            '--logfile', self.logfile]
        info("Executing '%s' in %s" % (command, self.tgz))
        if RunSudoAndWait(pbargs):
            err("pbuilder returned error. See log %s for details." % self.logfile)
            info("Done. '%s' was executed." % command)
...
class CentOSChroot():
    ...
    def init(self):
        ...
        if RunSudoAndWait(mockargs):
            err("mock returned error. See log %s for details." % os.path.join(self.temp_dir,
                                                                              'root.log'))
        info("Done")
    ...
    def exe(self, command):
        mockargs = ['mock', '--chroot', command,
            '--root', self.chroot,
            '--rootdir', self.rootdir,
            '--arch', self.arch,
            '--resultdir', self.temp_dir]
        info("Executing '%s' in %s" % (command, self.chroot))
        if RunSudoAndWait(mockargs):
            err("mock returned error. See log %s for details." % os.path.join(self.temp_dir,
                                                                              'root.log'))
        info("Done")
...
def main():
    ...
    groupCmd.add_argument('-x', '--execute', dest='command', action='store_const', const=CMD_EXE, help='Execute command in chroot')
    ...
    parser.add_argument('-c', '--command', dest='cmd', type=str, help='command to execute')
    ...
    cmd_messages = ['build package(s) for',
                    'init chroot for',
                    'update chroot for',
                    'login into',
                    'execute command for']
    info('Will {0} {1}'.format(cmd_messages[CMD],
                               DISTRO))
    ...
    if CMD == CMD_LOGIN:
        ...
        if not COMMAND:
            err('Command not specified, use --command argument!')
        chroot.exe(COMMAND)
    elif CMD == CMD_BUILD:
        if not SRC:
